...
class StarVote extends React.Component {
    constructor(props) {
        ...
        try {
            if (Boolean(this.state.allowHalf)) {//允许半选
            ...
            if (this.state.value) { //存在当前数
                if (!this.state.allowHalf) { // 不允许半选，count 、 value等验证必须为整数 且受控值 value 必须小于 cout
                ...
                } else { // 如果允许半选，存在当前数的情况 需要验证当前数是否是数值 必须是 0.5的倍数 且小于 count
            ...
            if (this.state.defaultValue) { //存在defaultValue
                if (!this.state.allowHalf) { // 不允许半选，defaultValue等验证必须为整数 且受控值 value 必须小于 cout
                ...
                } else { // 如果允许半选，存在defaultValue的情况 需要验证 defaultValue 是否是数值 必须是 0.5的倍数 且小于 count
    ...
    componentDidMount() {
        let tagArray = [];
        if (this.state.value) { // 如果存在当前数 按照当前数去渲染
            for (var i = 0; i < this.state.count; i++) {
                if (this.state.value.toString().includes('.5')) { //当前数为半数
        ...
        } else if (this.state.defaultValue) {// 不存在当前数 按照defaultValue 处理
            for (var i = 0; i < this.state.count; i++) {
                if (this.state.value.toString().includes('.5')) { //当前数为半数
    ...
    onMouseOver = (key, e) => {// li onMouseOver event
    ...
    onMouseOverContent = (key, e) => { //onMouseOver选一半
        e.preventDefault();
        e.nativeEvent.stopImmediatePropagation();
        e.stopPropagation();
        if (!this.state.disabled) {
    ...
    onMouseOut = (key, e) => { //onMouseOut 清除
        e.nativeEvent.stopImmediatePropagation();
        e.stopPropagation();
        if (!this.state.disabled) {
    ...
    render() {
            ...
            if (this.state.allowHalf) {// 允许半选
                if (tag == 1) {
                    return (<li key={index}
                        ...
                        <Star />
                        <div onMouseOver={(e) => this.onMouseOverContent(index, e)} onMouseOut={(e) => this.onMouseOutContent(index, e)} onClick={(e) => this.onClickContent(index, e)}
                             className={classes.rmVoteStarContent}></div>
                    </li>)
                } else if (tag == 0.5) {
                    return (<li key={index}
                        onMouseOver={(e) => this.onMouseOver(index, e)}
                ...
                if (tag == 1) {
                    return (<li key={index}
                        ...
                        className={classes.rmVoteStar +' '+ classes.rmVoteStarFull}>
                        <Star />
                        <div className={classes.rmVoteStarContent}></div>
                    </li>)
                } else if (tag == 0.5) {
                    return (<li key={index}
                        onMouseOver={(e) => this.onMouseOver(index, e)}
        ...
        if(this.state.disabled){
            return (
                <ul className={classes.rmVoteDisabled +' '+ classes.rmVote}>{listItems}</ul>
            );
        }else{
            return (
                <ul className={classes.rmVote}>{listItems}</ul>
            );
        }
