...
class _Panel extends React.Component {
  ...
  dragSourceResize = (event, direction, refToElement, delta) => {
    ...
    // 原始宽高
    const _h = refToElement.offsetHeight;

    const _w = refToElement.offsetWidth;
    // 改变的宽高
    const _dw = delta.width;

    const _dh = delta.height;
    const _col = Math.round(_w / cs);

    const _row = Math.round(_h / cs);
