...
class _DandD extends PureComponent{
    ...
    render(){     
        const {connectDragSource,monitor,children,classes,connectDropTarget,connect}=this.props;
        const childrenWithProps = React.Children.map(children, child =>
        React.cloneElement(child, { register: this.register.bind(this),accept:this.accept,connect:connect, monitor:monitor}));
        return connectDropTarget(connectDragSource(  <div className={classes.root}>
                {childrenWithProps}
           </div>))
    }
