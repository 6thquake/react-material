...
class TooltipElement extends React.Component {
    ...
    render() {
        return ReactDOM.createPortal(
            (<div className={`rm-tooltip ${!this.props.showTag && 'rm-tooltip-hidden'} rm-tooltip-placement-${this.props.placement}`} style={{ left: `${this.props.left}px`, top: `${this.props.top}px`}}>
    ...
    componentDidUpdate() {
        ...
        if (positionGroup.top.includes(this.state.placement)) {//top Group
            if(this.state.placement ==  "top"){//top
                if (parseInt(this.state.TooltipElementDomWidth) >= parseInt(this.state.elementWidth)) {//tooltip宽度大于基本元素宽度
                    let diff = parseInt(this.state.TooltipElementDomWidth) / 2 - parseInt(this.state.elementWidth) / 2;
                    this.state.styleLeft = this.state.styleLeft - diff;
                } else {
                    let diff = parseInt(this.state.elementWidth) / 2 - parseInt(this.state.TooltipElementDomWidth) / 2;
                    this.state.styleLeft = this.state.styleLeft + diff;
                }

            }else if(this.state.placement ==  "topRight"){//topRight
                if (parseInt(this.state.TooltipElementDomWidth) >= parseInt(this.state.elementWidth)) {//tooltip宽度大于基本元素宽度
                    let diff = parseInt(this.state.TooltipElementDomWidth) - parseInt(this.state.elementWidth) ;
                    this.state.styleLeft = this.state.styleLeft - diff;
                } else {
                    let diff = parseInt(this.state.elementWidth) - parseInt(this.state.TooltipElementDomWidth) ;
                    this.state.styleLeft = this.state.styleLeft + diff;
                }
            }
            ...
            if(this.state.placement ==  "right"){
                if (parseInt(this.state.TooltipElementDomHeight) >= parseInt(this.state.elementHeight)) {//tooltip宽度大于基本元素高度
                    let diff = parseInt(this.state.TooltipElementDomHeight) / 2 - parseInt(this.state.elementHeight) / 2;
                    this.state.styleTop = this.state.styleTop - diff;
                } else {
                    let diff = parseInt(this.state.elementHeight) / 2 - parseInt(this.state.TooltipElementDomHeight) / 2;
                    this.state.styleTop = this.state.styleTop + diff;
                }
            }else if(this.state.placement ==  "rightBottom"){
                if (parseInt(this.state.TooltipElementDomHeight) >= parseInt(this.state.elementHeight)) {//tooltip宽度大于基本元素高度
                    let diff = parseInt(this.state.TooltipElementDomHeight)  - parseInt(this.state.elementHeight) ;
                    this.state.styleTop = this.state.styleTop - diff;
                } else {
                    let diff = parseInt(this.state.elementHeight)  - parseInt(this.state.TooltipElementDomHeight) ;
                    this.state.styleTop = this.state.styleTop + diff;
                }
            }
