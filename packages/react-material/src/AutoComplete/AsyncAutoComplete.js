      ...
      items = options
        ? options.map((item, index) => {
            ...
            switch (typeof item) {
                  ...
                  if (!Array.isArray(value)) {
                    throw new Error(
                      'React-Material: the `value` property must be an array ' +
                        'when using the `AsyncAutoComplete` component with `multiple`.',
                    );
                  }
              ...
              default:
                throw new Error(
                  'AsyncAutoComplete[options] only supports type `string[] | Object[]`.',
                );
            }
          })
        : [];
...
export default withStyles(styles, { name: 'RMAsyncAutoComplete' })(AsyncAutoComplete);
