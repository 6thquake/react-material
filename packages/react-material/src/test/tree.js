...
class App extends Component {
  ...
  render() {
    ...
    const icons = {
      leftIconCollapsed: (
        <i style={{ height: 16, width: 16, color: '#CCCCCC' }} className="fa fa-caret-right" />
      ),
      leftIconExpanded: (
        <i style={{ height: 16, width: 16, color: '#CCCCCC' }} className="fa fa-caret-down" />
      ),
    };
          ...
          <div style={{ paddingLeft: 400 }}>
            <div style={{ width: 600, height: 400, margin: '20px auto' }}>
              ...
              <div style={{ marginTop: 20 }}>
                ...
                />
                <RaisedButton label="Next" secondary={true} onClick={this.moveToNext} />
              </div>
              <Card style={{ marginTop: 20 }}>
                <CardTitle title={listItems[activeListItem].title} />
                <CardText>{listItems[activeListItem].content}</CardText>
              </Card>
            </div>
          </div>
        </div>
...
function getAllParents(listItem, listItems, parents = []) {
  if (listItem.parentIndex) {
    return getAllParents(
      listItems[listItem.parentIndex],
      listItems,
      parents.concat([listItem.parentIndex]),
    );
  } else {
    return parents;
  }
